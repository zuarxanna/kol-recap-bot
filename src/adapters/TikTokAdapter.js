...
//
// Same shape as InstagramAdapter: scrape by username + date, RETURN pre-filter
...
export class TikTokAdapter extends PlatformAdapter {
  ...
  /** @returns {string} */
  get platform() { return 'TikTok'; }

  /**
   * @param {object} kol
   * @returns {boolean} True when the KOL has a TikTok username.
   */
  canHandle(kol) { return Boolean(kol.tiktok_username && kol.tiktok_username.trim()); }

  /**
   * @param {object} kol
   * @returns {string} Cleaned TikTok handle.
   */
  handleFor(kol) { return String(kol.tiktok_username || '').trim(); }
  ...
  async fetchContent(kol, campaign) {
    ...
    // WIDEN by 1 day: Apify filters in UTC, our date is WIB (+7). An early-morning
    // WIB post on campaign day 1 is the previous day in UTC, so without widening it
    // would be silently dropped. Over-fetching 1 day is safe: the hashtag filter and
    // WIB bucketing keep the output correct. Mirrors the console test that works.
    const since = TikTokAdapter.#minusOneDay(String(campaign.started_at || '').slice(0, 10));

    // Input MIRRORS the console test proven to work. Date param =
    // oldestPostDateUnified (NOT oldestPostDate — that name is silently ignored).
    ...
    const clean = items.filter((it) => !it.error); // drop error items (private/missing)
